        ...
        if (($user = Auth::user())->verifyRole(5)) {
            if (($station = Station::find($request->id)) != null) {
                ...
                if (($range = $station->vouchers->where('status', 4)->first()) != null) {
                    ...
                    $user->client->save();
                    return $this->successResponse('message', 'Al recoger tu vale presenta una identificación oficial en la estación. <br> <strong> Solo puedes recoger tu vale de lunes a viernes de 9 am a 7 pm. </strong>');
                }
                return $this->errorResponse('Intente más tarde');
            }
