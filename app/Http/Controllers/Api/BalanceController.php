...
class BalanceController extends Controller
{
    ...
    public function addPointsAlvic(Request $request)
    {
        ...
        if( $sale["station"] == "-1" ){
            if( $sale["validation"] == 409 ){
                ...
                return $this->response->errorResponse("Verifique sus datos.", 409);
            }
     ...
     // Metodo para sumar puntos QR o formulario
     public function addPointsAlvicNewSectionInvoice(Request $request)
     {
         $sale = "";
        ...
        //      $request->merge(['code' => $request->qr]);
         ...
         if( isset($request->uuid) ){
            ...
            //  if ($validator->fails()) {return $this->response->errorResponse($validator->errors()); }
             ...
             //Se verifica la informacion con alvic para proceder a la suma de puntos
             $sale = $this->getSaleByUuid( $request );
             // echo json_encode( $sale );
             Log::info(json_encode($request->all())); // Registra parametros de solicitud
        ...
        //  echo json_encode($sale);
         ...
         if( $sale["station"] == "-1" ){
             if( $sale["validation"] == 409 ){
                return $this->response->errorResponse("Verifica tu comprobante fiscal", 400);
             }
             return $this->response->errorResponse('Verifica tu comprobante fiscal', 400);
         }
        //      if( $sale["validation"] == 409 ){
     ...
     * @param object $request : Objeto que contiene la informacion enviada por post
     */
    private function getSaleByUuid( $request = null ){
        try {
            //Obtenemos el uid
            $uuid = trim($request->uuid);
            //Creamos el arreglo de parametros
            $params = array('uuid'=>$uuid);
            //Declaramos la url de peticion
            $host = "https://gasofac.mx/AlvicFac/api_sales/getSaleByUuid";
            //Iniciamos la configuracion de curl
            $curl = curl_init();
            curl_setopt($curl, CURLOPT_URL, $host);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1); 
            curl_setopt($curl, CURLOPT_TIMEOUT, 0);
